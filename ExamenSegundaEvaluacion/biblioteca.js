//⬇⬇ CAMBIOS REALIZADOS A PETICION DE LOS ENUNCIADOS DEL EXAMEN ⬇⬇
...
Sólo se modificará, si es necesario, el formato de numPrestamo, numSocio y/o cod Libro para que sea compatible con el formato elegido por el alumno para estas propiedades de acuerdo al código de la actividad ya entregado antes del examen.
Debajo del botón de su sección (Importar), una vez clickado, aparecerá un mensaje diciendo si se ha producido algún error en la importación y cuál ha sido (texto en color rojo) o un mensaje diciendo que se ha realizado la importación de forma exitosa (texto en color verde). El mensaje aparecerá cada vez que se pulse el botón Importar; si se vuelve a clickar el botón de esta sección, desaparecerá este mensaje
...
document
  ...
  .addEventListener("click", function () {
    ...
    // Si hay un archivo de prestamos, lo leo y proceso
    if (filePrestamos) {
      ...
      readerPrestamos.onerror = function () {
        errorOImportacionExitosa = true;
        mostrarMensajeError("Error al leer el archivo de prestamos.");
      };
      readerLectores.readAsText(fileLectores);
    }
...
function processCSV(text, tipoFichero) {
  ...
  if (tipoFichero === "lectores") {
    ...
    for (let celda of fichero) {
      listadoPrestamos.push(new Prestamo(celda[0], celda[1], celda[2]));
    }
    listadoPrestamos.sort((a, b) => a.numPrestamo - b.numPrestamo); //Ordeno los prestamos por el numero de prestamo de mayor a menor
  }
}
...
* Cuando el cursor salga de la sección Vista Libros, aparecerá en el elemento p con id="cursor-sobre-vista-libros" el mensaje: "Salío de Vista Libros".
...
En la especificación sobre la que ha trabajado el alumno, el teléfono del usuario (propiedad telefono)
...
function verificarTelefono(numeroTelefono) {
  ...
  const telefonoPrefijoValido = /^\+34\s[9|6|7][0-9]{8}$/; // Comienza por +34 seguido de un espacio y 9, 6 o 7 y tiene 8 dígitos despues del primero
  if (
    !telefonoValido.test(numeroTelefono) &&
    !telefonoPrefijoValido.test(numeroTelefono)
  ) {
    return false;
  } else {
    return true;
...
Si el alumno ha definido en la actividad entregada un array de objetos como estructura para la lista de libros: Se desarrollará una función llamada crear NuevaListaLibros que, teniendo 
como argumento la estructura que ya tiene creada el alumno, devuelva una nueva estructura llamada nuevo Objeto ListaLibros que será un objeto de objetos */
...
//Ejemplo de uso
const nuevoObjetoListaLibros = crearNuevaListaLibros(listaLibros);
console.table(nuevoObjetoListaLibros);
...
La lista con los libros se actualizará con el filtro al clickar en el botón Actualizar Libros (id= "vista-libros-boton")
...
document
  ...
  .addEventListener("click", function () {
    ...
    listaLibros.forEach((libro) => {
      if (libro.titulo.toLowerCase().includes(filtro) && !libro.bajaLibro) {
        tbody.innerHTML += `
        <tr>
...
La especificación para la función solicitudPrestamo 0 dice:
Un préstamo se solicita introduciendo los datos numSocio y codLibro. Se creará un nuevo préstamo, comprobándose antes si es posible realizarlo. Condiciones para realizar el préstamo:
tanto el lector como el libro existen y no están dados de baja; hay un número de ejemplares mayor que 0 de dicho libro.
...
solicitud Prestamo contendrá 3 funciones, cada una de ellas con una promesa. Se tendrá que cumplir una promesa para poder pasar a la siguiente. Estas funciones se llamarán desde solicitud Prestamo O de forma asíncrona.
Estas funciones realizarán las siguientes tareas:
* comprobarLibro0: Comprobará que el libro existe, que no está dado de baja y que hay un
número de ejemplares mayor que 0
* comprobar Lector0: Comprobar que el lector existe y que no está dado de baja
* generar Prestamo0: Creará un préstamo
...
function comprobarLibro(codLibro) {
  return new Promise((resolve, reject) => {
    ...
    if (libro && libro.ejemplares > 0) {
      ...
      reject(
        "El libro no existe, está dado de baja o no hay ejemplares disponibles."
      );
    }
  });
}
...
function comprobarLector(numSocio) {
  return new Promise((resolve, reject) => {
    const lector = listaLectores.find(
      (lector) => lector.numSocio === numSocio && !lector.bajaLector
    );
    if (lector) {
      resolve(lector);
    } else {
      reject("El lector no existe o está dado de baja.");
    }
  });
}

function generarPrestamo(libro, numSocio) {
  return new Promise((resolve, reject) => {
    const numPrestamo =
      listadoPrestamos.length > 0
        ? listadoPrestamos[listadoPrestamos.length - 1].numPrestamo + 1
        : 1;
    const nuevoPrestamo = new Prestamo(numPrestamo, numSocio, libro.codLibro);
...
function solicitudPrestamo(codLibro, numSocio) {
  ...
  comprobarLibro(codLibro)
    .then((libro) =>
      comprobarLector(numSocio).then(() => generarPrestamo(libro, numSocio))
    )
    .then((prestamo) => {
      mostrarMensajeExito(
...
//⬇⬇ CÓDIGO COMENTADO PARA QUE NO HAYA ERRORES ⬇⬇
/*function verificarTelefono(numeroTelefono) {
  const telefonoValido = /^[9|6|7][0-9]{8}$/; // Comienza por 9, 6 o 7 y tiene 8 dígitos despues del primero
